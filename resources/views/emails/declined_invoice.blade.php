...
<head>
    ...
    <title>Order: {{ $order->order_code }} is Declined!</title>
	...
	<div>
		...
		<div style="background: #eceff4;padding: 1rem;">
			...
			<table>
				...
				<tr>
					...
					<td class="text-right small"><span class="gry-color small">{{  translate('Order Date') }}:</span> <span class=" strong">{{ date('d-m-Y', $order->date_order_placed) }}</span></td>
				</tr>
			</table>
		...
		<div style="padding: 1rem;padding-bottom: 0">
            <table>
				...
				<tr><td class="gry-color small">{{ translate('Phone') }}: {{ $order->user->phone }}</td></tr>
			</table>
		</div>
			...
			<table class="padding text-left small border-bottom">
				<thead>
	                <tr class="gry-color" style="background: #eceff4;">
						...
						<th width="15%" class="text-left">{{ translate('Pickup Point Location') }}</th>
		                ...
		                @if ($orderDetail->product != null)
							<tr class="">
								<td>
                                    ...
                                    <br>
                                    {{ ucfirst(str_replace('_', ' ', $orderDetail->order_type)) }}
                                </td>
								<td>
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			            ...
			            <td class="currency">{{ single_price($order->order_declined_details->sum('price')) }}</td>
			        </tr>
			        <tr>
			            <th class="gry-color text-left">{{ translate('Shipping Cost') }}</th>
			            <td class="currency">{{ single_price($order->order_declined_details->sum('shipping_cost')) }}</td>
			        </tr>
			        <tr class="border-bottom">
			            <th class="gry-color text-left">{{ translate('Total Tax') }}</th>
			            <td class="currency">{{ single_price($order->order_declined_details->sum('tax')) }}</td>
			        </tr>
                    <tr class="border-bottom">
