...
@section('content')
    @if($order_id)
        <div class="container">
            <div style="padding: 20px; padding-left: 0;">
                <a class="d-flex align-items-center back-arrow text-gray c-pointer" href="{{ route('walkin.product', ['order_id' => $order_id])}} ">
    ...
    @endif
    <div class="breadcrumb-area breadcrumb-bg bg-lightblue">
        <div class="container">
            <div class="row">
                <div class="col breadcrumb-subcategory">
                    <ul class="breadcrumb">
                        <li>
                            <a href="@if($order_id) {{ route('walkin.product', ['order_id' => $order_id]) }} @else {{ route('walkin.product') }} @endif">{{ translate('Products') }}</a>
                        ...
                        @if (isset($category_id))
                            <li>
                                <a
                                    href="@if($order_id) {{ route('products.category' , ['category_slug' => \App\Category::find($category_id)->slug, 'order_id' => $order_id ]) }} @else {{ route('products.category', \App\Category::find($category_id)->slug) }} @endif">{{ translate(\App\Category::find($category_id)->name) }}</a>
                        ...
                        @endif
                        @php 
                            $categ_name = \App\Category::where('id', $detailedProduct->category_id)->get()->first();
                        @endphp 
                        <li>{{ $categ_name->name }}
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <div class="col-12 col-lg-6 mb-4">
                            <div class="text-left">
                                ...
                                <form id="option-choice-form">
                                    ...
                                    <hr class="d-lg-none d-md-none d-none">

                                    <div class="d-flex align-items-center">
                                        <div class="mr-2 w-100 w-md-50 d-lg-none d-md-none d-none">
                                    ...
                                    <div class="___class_+?63___ d-lg-none d-md-none d-none" id="pickup-address" style="display: none;">
                                    ...
                                    <div class="notif-success-bg d-lg-none d-md-none d-none" id="sameDayPickup" style="display: none;">
                                            ...
                                            <div class="no-gutters align-items-center aiz-plus-minus">
                                                ...
                                                @if ($order_id)
                                                    <button type="button"
                                                        ...
                                                        <span class="___class_+?90___"> {{ translate('Add to Order') }}</span>
                                                    </button>
                                                @else
                                                    <button type="button"
                                                        class="d-md-inline-block btn btn-craft-primary-nopadding add-to-cart fw-600 d-block fs-11"
                            ...
                            <div class="col col-sm-6 mb-4">
                                <div class="aiz-card-box border-product shadow-sm hov-shadowmd has-transition bg-white mx-1">
                                    <div class="position-relative">
                                        <div class="d-block">
                                            <a href="{{ route('walkin.product', $product->slug) }}" class="d-block">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#check-stock').on('change', function() {
                console.log('check-stock', check-stock);
                if ($('#check-stock').val() != "") {
                    $('#btnAddToCart').prop('disabled', true);
                    $('#btnAddToOrder').prop('disabled', true);
                    $('#available-quantity-stock').hide();
        ...
        function setLoc(){
            console.log('val',$('#check-stock').val());
            if($('#check-stock').val()){
                if ($('#check-stock').val() != "") {
                    $('#btnAddToCart').prop('disabled', true);
                    $('#btnAddToOrder').prop('disabled', true);
                ...
                AIZ.plugins.notify('warning', 'Not Available on this store');
            }
        ...
        function getStock() {
            ...
            var getPuke = getCookie('store_data');
            var myArri = getPuke ? JSON.parse(getPuke) : null;
            var myPenis = myArri ? myArri.name.toLowerCase().replace(' ', '_') : '{{$location}}'; 
            myPenis = myPenis ? myPenis.replace('+', '_') : '{{$location}}'; 
            document.getElementById('pup_loc_id').value = myArri.id;
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    productStock.forEach(function(item) {
                        ...
                        if(item.name.toLowerCase().replace(' ', '_').includes(myPenis)){
                            if (item.stock != null) {
                                ...
                                @else
                                    ...
                                    $('#check-stock').val(item.name.toLowerCase().replace(' ', '_'));
      ...
      function getCookie(cname) {
        ...
        $('#btnAddToCart').click(function() {
            addToCartWalkin(locationProductStock, handlingFee);
        });

        $('#btnAddToOrder').click(function() {
            addToOrderCashier(locationProductStock, handlingFee);
        });

        function addToOrderCashier($stock, $handlingFee) {
        if (checkAddToCartValidity()) {
            if ($('#check-stock').val() != "") {
                ...
                if ($stock.quantity > 0) {
                    shipping_type = "same_day_pickup";
                }

                else{
                    shipping_type = "advance_order";
                }
                ...
                data.push(
                    ...
                    {
                        'name': 'handlingFee',
                        'value': JSON.stringify($handlingFee)
                    }
                );
