                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-center align-items-center pt-4">
                                <div class="text-center mb-4 w-100 w-md-50">
                                    ...
                                    @php
                                        $decoded = json_decode($paynamics_query_request->paynamics_response);
                                        $response_code = $decoded->response_code;
                                        $response_message = $decoded->response_message;
                                    @endphp
    
                                    <div class="order-confirmed-title mb-3">
                                        @if ($response_code == 'GR001')
                                            Payment Successful!
                                        @elseif ($response_code == 'GR033')
                                            Payment Pending...
                                        @elseif ($response_code == 'GR003')
                                            Payment Failed!
                                        @elseif ($response_code == 'GR122')
                                            Transaction Not Found!
                                        @elseif ($response_code == 'GR152')
                                            Transaction Expired
                                        @endif
                                    </div>
                                    ...
                                    <div class="order-confirmed-subtitle">
                                        @if ($response_code == 'GR001')
                                            {{ $response_message }} for Cash in Transaction ID #{{ $wallet->transaction_id }}
                                        @elseif ($response_code == 'GR033')
                                        ...
                                        @elseif ($response_code == 'GR003')
                                            {{ $response_message }}! Your cash in request is now cancelled. You may request for cash in again and try another payment method.
                                        @elseif ($response_code == 'GR122')
                                            Transaction not found! Please try again, or you can process a new cash in request.
                                        @elseif ($response_code == 'GR152')
                                            Cannot process transaction because it was expired prior to payment
                                        @endif
                                    </div>
                            ...
                            @if ($response_code != 'GR003' && $response_code != 'GR001' && $response_code != 'GR152')
                                @php
                                    $paynamics_transaction = \App\PaynamicsTransactionRequest::where('type', 'wallet')
                                        ->where('notifiable_id', $wallet->id)
                                        ->first();
