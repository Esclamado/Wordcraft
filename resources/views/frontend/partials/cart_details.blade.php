@extends('frontend.layouts.cartapp')
                                                        ...
                                                        @if ($cartItem['digital'] != 1)
                                                            <div class="row no-gutters align-items-center aiz-plus-minus mr-2 ml-0 pt-2 pb-2"
                                                                ...
                                                                <input type="text" name="quantity[{{ $key }}]"
                                                                    ...
                                                                    max="{{ $worldcraft_stock_qty }}" readonly
                    ...
                    <div class="shadow-sm bg-white p-3 p-lg-4 rounded text-left">
                        ...
                        <div class="summary-body">
                            ...
                            @if (Session::has('coupon_discount'))
                                <div class="fw-400 fs-14 lh-21 float-right">
                                    {{ single_price(Session::get('coupon_discount')) }}</div>
                                <div class="text-subheader">{{ translate('Coupon Discount') }}</div>
                                <hr>
                            @endif
...
session_cart.forEach((data)=>{
    if($('#cart #checkbox_item #'+data.pickup_location+"_"+data.id+"_"+data.variant).is(':checked')){
        total += (parseInt(data.price)+parseInt(data.tax))*parseInt(data.quantity);
...
@foreach (Session::get('cart') as $key => $cartItem)
    if($('#cart #checkbox_item #'+'{{$cartItem['pickup_location']}}_{{$cartItem['id']}}').is(':checked')){
        total += (parseInt('{{$cartItem['price']}}')+parseInt('{{$cartItem['tax']}}'))*parseInt('{{$cartItem['quantity']}}');
   ...
   pickup_loc.forEach((data)=>{
    @foreach (Session::get('handlingFee') as $key => $itemStore)
              if ('{{ strtolower(str_replace(' ', '_', $itemStore->name)) }}' == data) {
                fee += parseInt({{$itemStore->handling_fee}});
              }
    @endforeach
   })
   ...
   if ( '{{Session::has('coupon_discount')}}')
   {
    grand_total -= '{{Session::get('coupon_discount')}}';
   }
