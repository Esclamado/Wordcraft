                    ...
                    <div class="col-md-6 col-12">
                        <div class="float-md-right">
                            <ul class="breadcrumb">
                                ...
                                <li class="active"><a href="{{ route('refund_request_lists') }}" class="text-breadcrumb text-secondary-color fw-600">{{ translate('My Returns') }}</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card card-craft-min-height border-0 shadow-none card-mobile-res1">
                                <div class="card-body overflow-auto">
                                    <h6 class="customer-craft-dashboard-card-title mb-0">
                                        Order Code: <span style="color: #161DBC;">{{ $request->order->code ?? "N/A" }}</span>

                                        <a href="{{ route('purchase_history.show', encrypt($request->order->id ?? "N/A" )) }}" class="customer-craft-dashboard-card-subtitle float-lg-right">
                                            View Order Details
                                        </a>
                                    </h6>
                                    <hr>
                                    <div class="row mb-3">
                                        <div class="col-3 col-lg-1">
                                            @php
                                                ...
                                                if ($request->orderDetail->product != null) {
                                                    if ($request->orderDetail->variation != "") {
                                                        $product_image = \App\ProductStock::where('product_id', $request->orderDetail->product_id)
                                                            ->where('variant', $request->orderDetail->variation)
                                                            ->first()->image;
                                                        ...
                                                        if ($product_image != null) {
                                                            ...
                                                            $product_image = uploaded_asset($request->orderDetail->product->thumbnail_img);
                                                        }
                                                    }
                                        ...
                                        <div class="col-9 col-lg-4">
                                            <div class="d-flex align-items-center h-100 craft-purchase-history-name">
                                                @if ($request->orderDetail->product != null)
                                                    <a href="{{ route('product', $request->orderDetail->product->slug ?? "N/A") }}" target="_blank">
                                                        {{ $request->orderDetail->product->getTranslation('name') ?? "N/A" }}

                                                        @if ($request->orderDetail->variation != null)
                                                            - {{ $request->orderDetail->variation }}
                                                        @endif
                                        ...
                                        <div class="col-12 col-lg-2 d-flex align-items-center justify-content-end">
                                            <div class="craft-purchase-history-quantity">
                                                <span class="opacity-50">Qty: </span><span style="color: #31303E">{{ $request->orderDetail->quantity ?? "N/A" }}</span>
                                            </div>
                                        </div>
                                        <div class="col-12 col-lg-5 d-flex align-items-center justify-content-end">
                                            <div class="craft-purchase-history-price">
                                                {{ single_price($request->orderDetail->price ?? "N/A") }}
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
                                    <div class="d-flex justify-content-between">
                                        <div class="align-items-start">
                                            <div class="d-flex justify-content-start">
                                                <div class="delivery-status @if ($request->refund_status == 1) delivery-status-picked-up @elseif ($request->refund_status == 2) delivery-status-rejected @else delivery-status-processing @endif mr-4">
                                                    @if ($request->refund_status == 0)
                                                        Pending
                                                    @elseif ($request->refund_status == 1)
                                                        Approved
                                                    @elseif ($request->refund_status == 2)
                                                        Rejected
                                                    @endif
                                                </div>
                                        ...
                                        <div class="align-items-end">
                                            ...
                                            <div class="float-right">
                                                <p class="@if ($request->orderDetail->payment_status == 'paid') text-success @else text-danger @endif mb-0">{{ ucfirst($request->orderDetail->payment_status ?? "N/A" ) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
