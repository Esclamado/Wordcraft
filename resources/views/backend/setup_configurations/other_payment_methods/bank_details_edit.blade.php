    ...
    <div class="col-lg-10 mx-auto">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">
                    {{ translate("Edit Bank Information") }}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('bank_lists.update', $bank_detail->id) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="form-group row">
                        <label for="parent_id" class="col-sm-3 control-label">Parent ID</label>
                        <div class="col-sm-9">
                            <select name="parent_id" id="" class="form-control aiz-selectpicker">
                                ...
                                @foreach ($other_payment_methods as $key => $value)
                                    <option value="{{ $value->id }}" {{ $bank_detail->other_payment_method_id == $value->id ? 'selected' : '' }}>{{ $value->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select name="pup_location_id" id="" class="form-control aiz-selectpicker" data-live-search="true">
                                ...
                                @foreach (\App\PickupPoint::where('pick_up_status', 1)->get()->pluck('id', 'name') as $location => $key)
                                    <option value="{{ $key }}" {{ $bank_detail->pickup_point_location == $key ? 'selected' : '' }}>{{ $location }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="false">
                                ...
                                <input type="hidden" name="bank_image" value="{{ $bank_detail->bank_image }}" class="selected-files">
                            </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="bank_name" class="form-control" placeholder="Bank Name" value="{{ $bank_detail->bank_name }}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="bank_acc_name" class="form-control" placeholder="Bank Account Name" value="{{ $bank_detail->bank_acc_name }}" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="bank_acc_number" class="form-control" placeholder="Bank Account Number" value="{{ $bank_detail->bank_acc_number }}" required>
                        </div>
                    </div>
