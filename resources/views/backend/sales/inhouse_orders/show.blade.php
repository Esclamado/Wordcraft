      ...
      <div class="card-header row gutters-5">
			...
			<div class="col-md-3 ml-auto">
                ...
                <select class="form-control aiz-selectpicker"  data-minimum-results-for-search="Infinity" id="update_delivery_status">
                    <option value="order_placed" @if ($delivery_status == 'order_placed') selected @endif>{{translate('Order Placed')}}</option>
                    <option value="confirmed" @if ($delivery_status == 'confirmed') selected @endif>{{translate('Confirmed')}}</option>
                    <option value="processing" @if ($delivery_status == 'processing') selected @endif>{{translate('Processing')}}</option>
                    <option value="partial_release" @if ($delivery_status == 'partial_release') selected @endif>{{translate('Partial Release')}}</option>
                    <option value="ready_for_pickup" @if ($delivery_status == 'ready_for_pickup') selected @endif>{{translate('Ready for Pickup')}}</option>
			...
			<div class="col-md-4 ml-auto">
              <table>
                  <tbody>
                    ...
                    <tr>
                        <td class="text-main text-bold">{{translate('Order Status')}}
                        ...
                        <td class="text-right">
                            @if($status == 'picked_up')
                                <span class="badge badge-inline badge-success" id="delivery_status">{{ translate(ucfirst(str_replace('_', ' ', $status))) }}</span>
                            @else
                                <span class="badge badge-inline badge-info" id="delivery_status">{{ translate(ucfirst(str_replace('_', ' ', $status))) }}</span>
                            @endif
                        </td>
                    </tr>
                    <tr>
            ...
            {{-- image receipt --}}
              <div>
                  <div class="card border-0 shadow-none">
                      <div class="card-header">
                          <h6 class="mb-0">{{ translate('Proof of Payment') }}</h6>
                      </div>
                      <div class="card-body">
                                          ...
                                          <td>
                                              @foreach ($photos as $key => $value)
                                                  <a href="{{ uploaded_asset($value) }}" target="_blank">
                                                      <img src="{{ uploaded_asset($value) }}" class="proof-of-payment mr-2" height="150" width="150" style="border: 1px solid #dee2e6; object-fit: cover;" alt="">
                                                  </a>
                                              @endforeach
                                          </td>
                                          <td>
                                              {{ date('m-d-Y', strtotime($proof_of_payment->paid_at)) }}
                                          </td>
                                      </tr>
                                  </tbody>
                              </table>
