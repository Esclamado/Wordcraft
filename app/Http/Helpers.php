...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text, $bulk_sms = false)
    {
         ...
         * @var [type]
         */
        elseif (OtpConfiguration::where('type', 'bulkgate_sms')->first()->value == 1) {
            ...
            if(Auth::user()->user_type == 'reseller' || Auth::user()->user_type == 'employee'){
                if($product->discount_type == 'percent'){
                    if(Auth::user()->user_type == 'reseller'){
                        $lowest_price -= ($lowest_price*$product->reseller_discount)/100;
                        $highest_price -= ($highest_price*$product->reseller_discount)/100;
                    }elseif(Auth::user()->user_type == 'employee'){
                        $lowest_price -= ($lowest_price*$product->employee_discount)/100;
                        $highest_price -= ($highest_price*$product->employee_discount)/100;
                    }
                }elseif($product->discount_type == 'amount'){
                    if(Auth::user()->user_type == 'reseller'){
                        $lowest_price -= $product->reseller_discount;
                        $highest_price -= $product->reseller_discount;
                    }elseif(Auth::user()->user_type == 'employee'){
                        $lowest_price -= $product->employee_discount;
                        $highest_price -= $product->employee_discount;
                    }
                }
            }else{
...
if (!function_exists('unique_code_exists')) {
    function unique_code_exists ($number) {
        return \App\User::where('unique_id', $number)->exists();
    }
}
...
if (!function_exists('unique_order_code_exists')) {
    function unique_order_code_exists ($number) {
        return \App\Order::where('unique_code', $number)->exists();
    }
}

if (!function_exists('unique_order_number')) {
    function unique_order_number () {
        $latest_order = \App\Order::latest()->first();
...
if (!function_exists('unique_order_number_exists')) {
    function unique_order_number_exists ($number) {
        return \App\Order::where('code', $number)->exists();
    }
} 

if (!function_exists('unique_order_number_cancelled_exists')) {
    function unique_order_number_cancelled_exists ($number) {
        return \App\OrderDeclined::where('order_code', $number)->exists();
    }
}
