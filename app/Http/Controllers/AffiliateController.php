...
class AffiliateController extends Controller
{
    ...
    public function affiliate_option_store(Request $request){
        ...
        if ($request->type == 'user_registration_first_purchase') {
            $affiliate_option->percentage = $request->percentage ?? null;
        }
        elseif ($request->type == 'employee_reseller_commission') {
            $affiliate_option->percentage = $request->percentage ?? null;
        }
        ...
        elseif ($request->type == 'minimum_first_purchase') {
            $affiliate_option->percentage = $request->percentage ?? null;
        }
        $affiliate_option->details = json_encode($commision_details);

        if ($request->type == 'minimum_first_purchase') {
    ...
    public function user_index(){
        ...
        $affiliate_withdraw_requests = AffiliateWithdrawRequest::where('user_id', Auth::user()->id)
            ->where('type', '!=', 'convert_to_wallet')
            ->orderBy('id', 'desc')
            ->paginate(10);
    ...
    public function processAffiliatePoints(Order $order){
        ...
        if(Addon::where('unique_identifier', 'affiliate_system')->first() != null && \App\Addon::where('unique_identifier', 'affiliate_system')->first()->activated){
            ...
            if(AffiliateOption::where('type', 'product_sharing')->first()->status){
                foreach ($order->orderDetails as $key => $orderDetail) {
                    ...
                    if($orderDetail->product_referral_code != null){
                        $referred_by_user_type = 'product_referral_code';
                        ...
                        if($referred_by_user != null) {
                            ...
                            if (AffiliateOption::where('type', 'employee_reseller_commission')->first()->status == 1) {
                                $percentage = AffiliateOption::where('type', 'employee_reseller_commission')->first()->percentage;
                                $employee_income = ($percentage * $orderDetail->price) / 100;
                            }
                        }
                    }
                    else {
                        $referred_by_user_type = 'registration_referral';
                        $referred_by_user = User::where('id', $order->user->referred_by)->first();
                    ...
                    $total_amount += $amount;
                    $total_employee_amount += $employee_income;
                }
            }
                ...
                foreach ($order->orderDetails as $key => $orderDetail) {
                    ...
                    if($orderDetail->product_referral_code != null) {
                        ...
                        if($referred_by_user != null) {
                            ...
                            $referred_by_user = User::where('id', $order->user->referred_by)
                                ->first();
            ...
            if ($referred_by_user != null) {
                if ($referred_by_user_type == 'product_referral_code') {
                    if ($referred_by_user->user_type == 'reseller') {
                        // Save Reseller Earnings
                        $reseller_earning = new ResellerEarning;

                        $reseller_earning->order_id = $order->id;
                        $reseller_earning->reseller_id = $referred_by_user->id;
                        $reseller_earning->customer_id = $order->user_id;
                        $reseller_earning->amount = $order->grand_total;
                        $reseller_earning->income = $total_amount;

                        $reseller_earning->paid_at = \Carbon\Carbon::now();
                        $reseller_earning->save();

                        // Save employee's reseller orders
                        $employee_reseller = EmployeeReseller::where('reseller_id', $reseller_earning->reseller_id)
                            ->first();

                        $employee_reseller->total_successful_orders += 1;
                        $employee_reseller->total_earnings += $total_amount;

                        $employee_reseller->save();

                        // Save employee earning if reseller is referred by employee
                        if ($referred_by_user->referred_by != null) {
                            $employee_earning = new EmployeeEarning;

                            $employee_earning->order_id = $order->id;
                            $employee_earning->employee_id = $referred_by_user->referred_by;
                            $employee_earning->reseller_id = $referred_by_user->id;
                            $employee_earning->customer_id = $order->user_id;
                            $employee_earning->order_code = $order->code;
                            $employee_earning->amount = $order->grand_total;
                            $employee_earning->income = $total_employee_amount;

                            $employee_earning->type = 'reseller_earning';
                            $employee_earning->paid_at = \Carbon\Carbon::now();

                            $employee_earning->save();
                        }
                    }

                    else if ($referred_by_user->user_type == 'employee') {
                        $employee_earning = new EmployeeEarning;

                        $employee_earning->order_id = $order->id;
                        $employee_earning->employee_id = $referred_by_user->id;
                        $employee_earning->customer_id = $order->user_id;
                        $employee_earning->order_code = $order->code;
                        $employee_earning->amount = $order->grand_total;
                        $employee_earning->income = $total_employee_amount;

                        $employee_earning->type = 'customer_earning';
                        $employee_earning->paid_at = \Carbon\Carbon::now();
                ...
                else if ($referred_by_user_type == 'registration_referral') {
                    $employee_earning = new EmployeeEarning;
                    ...
                    if ($order->user->user_type == 'reseller') {
                        $employee_earning->reseller_id = $order->user_id;
                        $employee_earning->type = 'reseller_earning';
                    }
    ...
    public function affiliate_withdraw_modal(Request $request)
    {
        ...
        $affiliate_user = AffiliateUser::where('user_id',$affiliate_withdraw_request->user_id)->exists();
        ...
        if ($affiliate_user) {
            ...
            $affiliate_user = User::where('id', $affiliate_withdraw_request->user_id)->first();
        }
    ...
    public function withdraw_request_payment_store(Request $request){
        ...
        if ($request->has('affiliate_withdraw_request_id')) {
            ...
            $wallet = \App\Wallet::where('id', $affiliate_withdraw_request->wallet_id)
                ->first();
    ...
    public function reject_withdraw_request($id)
    {
        ...
        if ($affiliate_withdraw_request->save()){
            ...
            $wallet = \App\Wallet::where('affiliate_withdraw_request_id', $affiliate_withdraw_request->id)
                ->first();
