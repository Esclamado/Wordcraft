...
class BusinessSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            ...
            if($type == 'widget_one') {
                $business_settings = BusinessSetting::where('type', 'widget_one')->first();
                $business_settings->value = $request->widget_one;
                $business_settings->save();

                foreach($request->widget_one_labels as $key => $val) {
                    $linkInfo = Link::where('name', $val)->where('url', $request->widget_one_links[$key])->first();

                    if ($linkInfo != null) {
                        if ($linkInfo['name'] != $val) {
                            $linkData = new Link();
                            $linkData->name = $val;
                            $linkData->url = $request->widget_one_links[$key];
                            $linkData->save();
                        }
                    }
